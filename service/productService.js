...
const getAllProduct = async (sort, limit, offset) => {
  ...
  if (sort === "name")
    products = await db.connection.execute(
      `SELECT * FROM product order by product.name asc LIMIT ${offset}, ${limit}`
    );
  else if (sort === "price")
    products = await db.connection.execute(
...
const getProductsByName = async (name, sort, limit, offset) => {
  ...
  if (sort === "name") {
    products = await db.connection.execute(
      `SELECT * FROM product WHERE product.name like ? order by product.name asc LIMIT ${offset}, ${limit}`,
      [`%${name}%`]
    );
  } else if (sort === "price") {
    products = await db.connection.execute(
...
const getProductsByCategory = async (category, sort, limit, offset) => {
  ...
  if (sort === "name") {
    products = await db.connection.execute(
      `select * from product where product.category like ? order by product.name asc LIMIT ${offset}, ${limit}`,
      [`%${category}%`]
    );
  } else if (sort === "price") {
    products = await db.connection.execute(
...
const getProductsByManufacture = async (manufacture, sort, limit, offset) => {
  ...
  if (sort === "name") {
    products = await db.connection.execute(
      `select * from product where product.manufacture like ? order by product.name asc LIMIT ${offset}, ${limit}`,
      [`%${manufacture}%`]
    );
  } else if (sort === "price") {
    products = await db.connection.execute(
...
const addProductService = async (name, price, description, category, manufacture, create_at, image, status) => {
  const result = await db.connection.execute('INSERT INTO product(name, price, description, image, category, manufacture, create_at, status, numPurchased, sale) VALUES(?,?,?,?,?,?,?,?,?,?)',[name, price, description,image, category, manufacture,create_at,status,0,0])
  return result
